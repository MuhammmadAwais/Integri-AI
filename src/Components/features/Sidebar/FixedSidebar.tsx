...
import { setActiveSidebarTab, setContextSidebarOpen } from "../../../store/chatSlice";
...
const SidebarIcon: React.FC<SidebarIconProps> = ({ icon: Icon, label, isActive, showTooltip, onClick }) => {
  ...
  return (
    <button
      ...
      className={cn(
        ...
        isActive 
          ? (isDark ? "bg-indigo-600 text-white shadow-lg" : "bg-black text-white shadow-md") 
          : (isDark ? "text-gray-400 hover:bg-[#2A2B32] hover:text-white" : "text-gray-500 hover:bg-gray-100 hover:text-black")
      )}
    >
...
const FixedSidebar: React.FC = () => {
  ...
  const isContextOpen = useAppSelector((state) => state.chat.isContextSidebarOpen);

  // Helper to handle navigation + state + drawer logic
  const handleNav = (tab: "home" | "history" | "library" | "settings", path: string) => {
    
    // 1. Navigate first (Fixes the "Library page won't change" bug)
    navigate(path);

    // 2. If clicking the ALREADY active tab, just toggle the drawer
    if (activeTab === tab) {
  ...
  return (
    <div className={cn("flex flex-col items-center justify-between w-[72px] h-full py-6 border-r z-40 shrink-0 relative transition-colors", isDark ? "bg-[#171717] border-[#2A2B32]" : "bg-[#F9F9FA] border-gray-200")}>
      <div className="flex flex-col items-center w-full gap-2">
        ...
        <SidebarIcon 
          icon={Home} 
          label="Home" 
          isActive={activeTab === 'home' || location.pathname.startsWith('/chat')} 
          showTooltip={!isContextOpen}
          onClick={() => handleNav('home', '/')} 
        />
        
        {/* HISTORY - Now navigates to /history */}
        <SidebarIcon 
          icon={History} 
          label="History" 
